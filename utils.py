...
class AudioAugs:
    ...
    def add_sine_wave(self, sample):
        amplitude = 0.01  # Sine wave amplitude
        frequency = random.uniform(300, 600)  # Frequency in Hz
        
        # Ensure the time array matches the sample length
        time = torch.arange(0, sample.shape[-1]) / self.fs  # Adjust based on the actual time dimension
        
        sine_wave = amplitude * torch.sin(2 * math.pi * frequency * time)
        sine_wave = sine_wave.to(sample.device)  # Ensure the sine wave is on the same device as the sample

        # Reshape sine wave to match sample dimensions if necessary
        if len(sine_wave.shape) < len(sample.shape):
            sine_wave = sine_wave.view(1, -1).expand_as(sample)
    ...
    def pitch_shift(self, sample):
        ...
        shift_steps = random.randint(-5, 5)
        n_fft = 1024  # Modify n_fft based on your typical sample length

        # Ensure the n_fft is smaller than the sample length
        if sample.shape[-1] < n_fft:
            n_fft = sample.shape[-1]

        hop_length = n_fft // 4  # Ensure hop_length is appropriate based on n_fft

        # Apply pitch shift only if the sample length is sufficient
        if sample.shape[-1] > n_fft:
            pitch_shift = T.PitchShift(self.fs, n_steps=shift_steps, n_fft=n_fft, hop_length=hop_length)
            sample = pitch_shift(sample)
        else:
            # Handling for short samples; you could also choose to pad the sample here
            print("Sample too short for pitch shifting, applying no change.")
...
# import torch.nn.functional as F
# from scipy.sparse import coo_matrix
# from aug_helper import AugBasic
